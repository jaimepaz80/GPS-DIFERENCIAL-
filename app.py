...
# --- RUTA DINÁMICA DE TRABAJO EN ENTORNO SERVERLESS ---
UPLOAD_FOLDER = '/tmp'
REPORT_FOLDER = '/tmp'
...
# =====================================================================
# RUTAS FLASK (FLUJO ARQUITECTÓNICO ORIGINAL - LECTURA/ESCRITURA)
# =====================================================================
@app.route('/')
...
def tab2_efemerides():
    ...
    nav_filename = nf.filename
    nav_p = os.path.join(UPLOAD_FOLDER, nav_filename)
    nf.save(nav_p)
    ...
    def procesar():
        try:
            ...
            final_nav_p = nav_p
            # Soporte para descompresión de archivos .gz (Pesa 1.36 MB, ideal para Vercel)
            if nav_p.endswith('.gz'):
                yield f"  > Descomprimiendo archivo {nav_filename}...\n"
                unzipped_p = nav_p[:-3]
                with gzip.open(nav_p, 'rb') as f_in, open(unzipped_p, 'wb') as f_out:
                    shutil.copyfileobj(f_in, f_out)
                final_nav_p = unzipped_p
            
            yield f"  > Procesando efemérides...\n"
            nav_data = parse_rinex_nav_real(final_nav_p)
            if not nav_data or len(nav_data) <= 1:
                yield "> [ERROR] No se pudieron extraer efemérides válidas del archivo.\n"
                return
            
            guardar_estado('nav_path', final_nav_p)
            guardar_estado('name_nav_file', os.path.basename(final_nav_p))
            
            yield f"> [ÉXITO] Archivo de efemérides almacenado y validado.\n\n[SUCCESS]"
